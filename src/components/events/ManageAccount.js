  ...
  _rObj(){
      return(
          <div>
              ...
              <div>
                    Your updated profile details: <br/>

                    Name:  {this.props.auth.auth.user.profile.name}  <br/>
                    Email:  {this.state.student_info.email}  <br/>
                    Organization:  {this.state.student_info.organization}  <br/>
                    Designation:  {this.state.student_info.designation}  <br/>
  ...
  render() {
    ...
    if(!this.props.auth.auth.isAuthenticated){
      ...
      return(
        ...
        <Header />
             <Sidebar {...this.props} style={{ margin:'50px 0 0 0', background:"#e9f5ff"}} />
                  ...
                  <Row style={{width:"100%", fontSize:"2.3rem", letterSpacing:"1px",  color:"#777777", margin:"0"}}>
                      <Col xs="12" sm="12" lg="12" style={{padding:"20px", minHeight:"1000px"}} >
                          {this.state.student_info.email?(<div>
                              Manage your profile
                              {this.state.thanks_update  ?(
                                 <div>
                                     {this._rObj()}
                                 </div>

                              ):(
                                  <CustomerEmail  email={this.state.student_info.email} organization ={this.state.student_info.organization}  designation ={this.state.student_info.designation}
                                      name= {this.props.auth.auth.user.profile.name}  onSubmit={this.customer_info_update.bind(this)} />
                              )}


                          </div>):(null)}

                          <span style={{fontSize:"1.5rem"}}> We can add more input fields later </span>
                      </Col>
                  </Row>
